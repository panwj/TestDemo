...
import com.czt.mp3recorder.util.AudioRecorderPcm;
...
public class MP3RecordManager {
    ...
    private MP3RecordManager(Context context) {
        mContext = context;
        mRecordMode = (int) SharedPreferencesUtil.get(mContext, "record_mode", 0);
    };

    public static MP3RecordManager getInstance(Context context) {
        if (mInstance == null) {
            synchronized (MP3RecordManager.class) {
                if (mInstance == null) {
                    mInstance = new MP3RecordManager(context);
                }
            }
        }
        return mInstance;
        ...
        mp3MediaRecorder.setErrorHandler(new Handler() {
            ...
            public void handleMessage(Message msg) {
                ...
                if (msg.what == MP3MediaRecorder.ERROR_TYPE) {
                    ...
                    Toast.makeText(mContext, "media recording error", Toast.LENGTH_SHORT).show();
                    resolveError();
                }
            }
        });
