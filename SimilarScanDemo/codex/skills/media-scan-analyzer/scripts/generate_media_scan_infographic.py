...
class Svg:
    ...
    def wrapped_text(
        ...
        for raw in value.split("\n"):
            if not raw:
                ...
                # 技术图里会出现 READ_MEDIA_IMAGES、NORMAL_FRAME_COUNT 等长常量。
                # 这里允许长词按字符折行，避免 SVG 文本越过卡片边界。
                lines.extend(wrap(raw, width_chars, break_long_words=True, break_on_hyphens=False))
    ...
    def bullet_list(
        ...
        current = y
        for item in items:
            if max_y is not None and current + line_height > max_y:
                self.text(x, current, "…", size, color, "700")
                return current + line_height
            self.text(x, current, "•", size, color, "700")
            before = current
            current = self.wrapped_text(x + 16, current, item, width_chars, size, color, line_height)
            if max_y is not None and current > max_y:
                self.text(x + 16, before, "内容较多，详见文档", size, color, "400")
                return max_y
        return current
...
def step_cards(svg: Svg) -> None:
    ...
    titles = [
        ("授权", LIGHT_BLUE, BLUE, ["申请权限", "IMAGES", "VIDEO"]),
        ...
        ("入库", LIGHT_RED, RED, ["media_asset", "state = ACTIVE", "fp_status"]),
...
def main_sections(svg: Svg) -> None:
    ...
    row1_h = 350
    # Row 1
    svg.card(M, y, 380, row1_h, "2. 扫描模式", LIGHT_BLUE, BLUE, "2")
    svg.bullet_list(M + 25, y + 72, [
        ...
        "完整授权的全量扫描结束后清理未出现资源。",
    ], 31, 16, max_y=y + row1_h - 24)

    svg.card(M + 400, y, 520, row1_h, "3. 资源读取流程（批次大小：500）", "#fff", BLUE, "3")
    batch_x = M + 425
    for i in range(5):
        bx = batch_x + i * 96
    ...
    svg.wrapped_text(M + 430, y + 298, "500 只是读取批次，不是相似比较边界；扫描完成后统一重建 SimilarGroup。", 42, 15, GRAY, 20)

    svg.card(M + 940, y, 404, row1_h, "4. 媒体分类规则", "#fff", BLUE, "4")
    ...
    ], 34, 16, max_y=y + row1_h - 24)

    svg.card(M + 1364, y, 390, row1_h, "5. 权限与启动链路", LIGHT_GREEN, GREEN, "5")
    svg.bullet_list(M + 1389, y + 72, [
        "媒体权限通过后标记 pendingScanAfterPermission。",
        "通知权限只请求一次；拒绝不阻止扫描。",
        "startForegroundService 后 2s grace 避免 UI 回到 Rescan。",
        "扫描进度通过包内广播回首页。",
    ], 32, 16, max_y=y + row1_h - 24)

    # Row 2
    y2 = 740
    row2_h = 390
    svg.card(M, y2, 340, row2_h, "6. 图片指纹", "#fff", BLUE, "6")
    ...
    ], 29, 16, max_y=y2 + row2_h - 24)

    svg.card(M + 360, y2, 360, row2_h, "7. 图片相似识别流程", LIGHT_BLUE, BLUE, "7")
    cx = M + 450
    ...
    svg.card(M + 740, y2, 400, row2_h, "8. 相似阈值", "#fff", BLUE, "8")
    ...
    svg.wrapped_text(M + 765, y2 + 250, "直接区间命中即相似；中间距离继续用 colorHash 过滤。高距离直接不相似。", 30, 16, GRAY, 23)

    svg.card(M + 1160, y2, 594, row2_h, "9. 重复 / 相同图片识别", LIGHT_GREEN, GREEN, "9")
    ...
    ], 52, 16, max_y=y2 + row2_h - 24)

    # Row 3
    y3 = 1170
    row3_h = 410
    svg.card(M, y3, 570, row3_h, "10. 视频指纹", "#fff", BLUE, "10")
    ...
    ], 49, 16, max_y=y3 + row3_h - 24)

    svg.card(M + 590, y3, 570, row3_h, "11. 视频相似识别方案", LIGHT_BLUE, BLUE, "11")
    ...
    svg.card(M + 1180, y3, 574, row3_h, "12. 分组生成规则", "#fff", ORANGE, "12")
    ...
    y4 = 1620
    row4_h = 370
    svg.card(M, y4, 430, row4_h, "13. 资源状态", "#fff", BLUE, "13")
    ...
    svg.card(M + 450, y4, 430, row4_h, "14. 删除安全机制", LIGHT_ORANGE, ORANGE, "14")
    svg.bullet_list(M + 475, y4 + 72, [
        "markDeletePending：state=DELETE_PENDING，revision+1。",
        "提交时校验 token revision。",
        "删除确认后 finalizeDelete。",
        "取消或恢复时 restore/recover 到 ACTIVE。",
    ], 36, 16, max_y=y4 + row4_h - 24)

    svg.card(M + 900, y4, 430, row4_h, "15. 首页展示与性能", LIGHT_GREEN, GREEN, "15")
    ...
    svg.card(M + 1350, y4, 404, row4_h, "16. 当前边界", LIGHT_RED, RED, "16")
    svg.bullet_list(M + 1375, y4 + 72, [
        "外部相册删除在增量扫描中可能延迟同步。",
        "视频相似受静态 UI、片头、低信息帧影响。",
        "DATA 路径不可读会得到无效视频指纹。",
        "Other 详情超大集合后续建议分页。",
    ], 34, 16, max_y=y4 + row4_h - 24)

    # Footer summary
    y5 = 2030
    footer_h = 820
    svg.card(M, y5, W - 2 * M, footer_h, "17. 技术方案总结与审核点", "#fff", BLUE, "17")
    ...
    svg.bullet_list(M + 35, y5 + 295, [
        ...
        "Other 分类详情页是否需要分页加载。",
        "是否将最终图作为 PNG 交付；当前 skill 会优先用浏览器按完整画布渲染 PNG。",
    ], 100, 17, max_y=y5 + 465)
    svg.text(M + 35, y5 + 500, "生成物", 22, GREEN, "800")
    svg.wrapped_text(M + 35, y5 + 540, "审核稿：docs/generated/media-scan-analyzer/review/media_scan_infographic_review.svg/png；最终稿：docs/generated/media-scan-analyzer/final/media_scan_infographic_final.svg/png", 125, 17, GRAY, 26)
...
def try_export_png_with_playwright(svg_path: Path, png_path: Path) -> bool:
    """Export SVG to a full-size PNG through a headless browser.

    Quick Look may create a square thumbnail and crop the lower part of a tall
    SVG. Browser screenshot keeps the SVG viewBox size, so it is preferred.
    """
    ...
    module_candidates = [
        bundled_root / "node/node_modules",
    ]
    browser_candidates = [
        None,
...
(async () => {{
  const launchOptions = {{ headless: true {executable_path} }};
  const browser = await chromium.launch(launchOptions);
  const page = await browser.newPage({{ viewport: {{ width: {W}, height: {H} }}, deviceScaleFactor: 1 }});
  await page.goto({svg_uri});
...
"""
    for node in node_candidates:
        ...
        module_paths = [str(path) for path in module_candidates if path.exists()]
        if module_paths:
            env["NODE_PATH"] = os.pathsep.join(module_paths + ([existing_node_path] if existing_node_path else []))
        for browser_path in browser_candidates:
            if browser_path is not None and not browser_path.exists():
                continue
            executable_path = ""
            if browser_path is not None:
                executable_path = f", executablePath: {json.dumps(str(browser_path))}"
            script = script_template.format(
                executable_path=executable_path,
                W=W,
                H=H,
                svg_uri=json.dumps(svg_path.as_uri()),
                png_path=json.dumps(str(png_path)),
            )
...
def try_export_png_with_quicklook(svg_path: Path, png_path: Path) -> bool:
...
def try_export_png(svg_path: Path, png_path: Path) -> bool:
    if try_export_png_with_playwright(svg_path, png_path):
        return True
    return try_export_png_with_quicklook(svg_path, png_path)
