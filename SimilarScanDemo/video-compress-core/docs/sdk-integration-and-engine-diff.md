...
- 同一时间只压缩一个视频，这是 SDK 的固定约束。
- SDK 不提供并发压缩入口，避免多个视频同时抢占硬件编码器。
...
| 三档 bitrate | 支持 | 支持 |
| 压缩前真实 metadata | 支持，当前视频开始压缩前读取真实码率、帧率、宽高、旋转 | 基础支持，仅用于格式边界判断 |
| 分辨率/帧率质量保护 | 支持，4K/1080p/720p/540p 有独立 bitrate 下限，并结合帧率、HEVC 来源做保护 | 当前不做 |
| 分辨率降档 | 支持 | 当前不做 |
| 低收益压缩拦截 | 支持 | 当前不做 |
| 输出结果校验 | 支持 | 基础保存结果 |
