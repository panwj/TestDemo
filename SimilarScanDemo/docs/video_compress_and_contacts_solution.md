...
- 已支持磁盘空间预检查。
- 已支持前台服务承载长视频压缩。
- SDK 层已保持单任务顺序队列，不做并发压缩，降低硬件编码器资源抢占风险。
- Demo 前台服务已支持压缩中继续追加任务，新增任务会进入等待队列。
- 通知栏进度条展示整个队列总进度，通知文案中的 Current 表示当前单个视频进度，Queue 表示整个队列总进度。
...
如果用户在压缩过程中继续选择视频，Demo 前台服务会追加到等待队列，当前视频不会被中断。
