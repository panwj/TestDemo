...
扫描中不会每发现一条相似关系就直接维护 `similar_group`，而是先写入 `similar_candidate_edge`。为了让 UI 在扫描中能阶段性看到结果，扫描线程会按节流策略把候选边中间 publish 到 `similar_group`；扫描完成后再调用：

```text
ScanDatabase.rebuildSimilarGroups(processedKinds)
```

当前中间 publish 默认开启，demo 构建扫描参数时使用：
...
普通进度只代表扫描数量和耗时变化；只有中间 publish 成功或最终扫描完成时，`ScanProgress.resultUpdated=true`，首页才重新加载分组结果。
