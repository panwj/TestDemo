...
| 能力 | 当前支持 | 说明 |
| --- | --- | --- |
| 系统媒体枚举 | 支持 | 读取 `MediaStore.Images` 和 `MediaStore.Video`，不读取音频。 |
| 媒体入库类型 | 支持 | 入库后保留 `PHOTO`、`SCREENSHOT`、`VIDEO`、`SCREEN_RECORDING` 四类。 |
| 相同图片/截图 | 支持 | 输出到 `DUPLICATES`，包含普通照片和截图。 |
| 相似图片 | 支持 | 普通照片输出到 `SIMILAR`。 |
| 相似截图 | 支持 | 截图输出到 `SIMILAR_SCREENSHOTS`。 |
| 相似视频/录屏 | 支持 | 视频和录屏都归并输出到 `SIMILAR_VIDEOS`。 |
| 未命中截图 | 支持 | 输出到 `OTHER_SCREENSHOTS`。 |
...
- 当前 Demo 扫描服务使用 `BALANCED`，优先复用系统视频缩略图并补充少量 MMR 时间点，作为速度和召回之间的默认折中。
