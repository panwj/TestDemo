...
| `videoFingerprintMode` | `BALANCED` | 视频指纹模式：`FAST`、`BALANCED`、`ACCURATE`、`REFERENCE_COMPAT`、`ADAPTIVE_BALANCED` |
| `enableIntermediateGroupPublish` | `true` | 是否允许扫描中低频 DB 中间发布；默认作为 progressive snapshot 之外的兜底 |
| `firstIntermediateGroupPublishIntervalMs` | `60_000` | 首次 DB 中间发布最小等待时间 |
| `firstIntermediateGroupPublishMinAssets` | `5_000` | 首次 DB 中间发布最小新增扫描资源数 |
| `intermediateGroupPublishIntervalMs` | `90_000` | 后续 DB 中间发布最小时间间隔 |
| `intermediateGroupPublishMinAssets` | `10_000` | 后续 DB 中间发布最小新增扫描资源数 |
| `intermediateGroupPublishMinEdges` | `20_000` | 后续 DB 中间发布最小新增候选边数 |
| `maxIntermediateGroupPublishCount` | `2` | 单次扫描最多执行几次 DB 中间发布 |
...
| `ADAPTIVE_BALANCED` | 系统缩略图在部分设备上慢失败的场景 | 先尝试系统缩略图；连续失败、成功率低或耗时过高后，本轮直接使用 BALANCED 4 帧 MMR |

需要更高视频召回时，宿主产品可以显式传入 `VideoFingerprintMode.REFERENCE_COMPAT`；常规场景建议保留默认
`BALANCED`，或在已知设备存在系统缩略图慢失败时选择 `ADAPTIVE_BALANCED`。
...
-> resultUpdated=false 时 UI 只更新进度
-> resultUpdated=true 时 UI 节流读取 progressive snapshot 或最终 DB 结果
```

`progress.resultUpdated` 是结果刷新信号，不是普通进度信号。`false` 只建议更新扫描数量、耗时和进度条；`true` 表示 SDK 已发布新的 progressive snapshot、DB 中间结果或最终结果，宿主可以刷新首页/详情数据。

扫描中 UI 建议优先读取 progressive snapshot，而不是依赖 DB 中间发布：

```kotlin
if (progress.resultUpdated) {
    val categories = client.loadProgressiveProductCategories(previewAssetLimit = 2)
}
```

DB 中间发布默认仍开启，但定位是低频兜底，不是扫描中快速刷新的主路径。
            ...
            try {
                val result = client.scan(
                    ...
                    SimilarScanObserver { progress ->
                        // update notification
                        // publish progress to UI; refresh data only when progress.resultUpdated
                    }
                )
...
收到扫描进度
-> 更新状态文案
-> resultUpdated=false 时只更新数量/耗时/进度条
-> resultUpdated=true 时节流读取 progressive snapshot + DB cached fallback
...
display = progressive 中 itemCount>0 的分类覆盖 cached；其余分类继续展示 cached
```

这样可以让用户尽快看到本轮扫描发现的阶段性分组，同时避免 snapshot 尚未覆盖的分类突然清空。扫描完成后，首页应统一读取 `loadProductCategories(previewAssetLimit = 2)` 展示最终 DB 结果。
...
扫描中进入详情页时，推荐先读当前分类 snapshot：

```kotlin
val category = if (isScanning) {
    client.loadProgressiveProductCategory(type, previewAssetLimit = 60)
        ?.takeIf { it.itemCount > 0 }
        ?: client.loadProductCategory(type, previewAssetLimit = 60)
} else {
    client.loadProductCategory(type, previewAssetLimit = 60)
}
```

分组详情中 `groupId < 0` 表示扫描中的 snapshot 临时分组，SDK 的 `loadSimilarGroupAssets()` 已兼容该路径；`groupId > 0` 表示最终 DB 分组。
...
- progressive snapshot 是进程内临时数据，扫描完成、失败或 client close 后会清空；最终结果必须以 DB 读取为准。
- 扫描中设备新增/外部删除媒体不要求本轮立即一致；下次用户主动重扫或重新进入后触发扫描即可通过 MediaStore 对账更新。
...
- 扫描中收到 `resultUpdated=true` 时，首页优先读取 `loadProgressiveProductCategories(previewAssetLimit = 2)` 并使用 DB 缓存兜底。
- 分类详情读取 `client.loadProductCategory(categoryType)`，只加载当前分类的完整资源。
- 扫描中详情页优先读取 `loadProgressiveProductCategory(categoryType)`，无数据时回退 DB 分类。
